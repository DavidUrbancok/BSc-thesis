...
namespace CMS.Tests
{
    ...
    public abstract class AutomatedTestsWithLocalDB : AutomatedTestsWithData
    {
        ...
        private const string ExceptionMessageFormat_InstallationFailed = @"
            ...
            If there was a problem with the CMS.IO subsystem, it might be caused by the missing reference to the CMS.FileSystemProvider project (or assembly) in your test project.";
        ...
        /// <summary>
        /// <para>Indicates if database is created before OneTimeSetUp.</para>
        /// <para>(Instance database for each test is copied from it)</para>
        /// </summary>
        private readonly bool mCreateDatabaseBeforeTests;
        ...
        protected AutomatedTestsWithLocalDB()
        {
            ...
            // Check whether to use shared database
            var sharedDbAttributes = type.GetCustomAttributes(typeof(SharedDatabaseForAllTestsAttribute), true);
            SharedDatabaseForAllTests = (sharedDbAttributes.Any());

            var crateDbBeforeAttributes = type.GetCustomAttributes(typeof(CreateDatabaseBeforeTestsAttribute), true);
            mCreateDatabaseBeforeTests = (crateDbBeforeAttributes.Any());
        }
        ...
        public void AutomatedTestsWithLocalDBSetup()
        {
            ...
            {
                if (mCreateDatabaseBeforeTests)
                {
                    if (mTestDatabase == null)
                    {
                        DetachDatabase(mFixtureDatabase);
                    }

                    EnsureLocalDatabases(mFixtureDatabase.Name);
                }
                else
                {
        ...
        /// </summary>
        private void ReleaseDatabases(DatabaseProperties database)
        {
            try
            {
                if (database != null)
...
",
        ...
        private void DetachDatabase(DatabaseProperties database)
        {
            if (database.Detached)
            {
                return;
            }
        ...
        private static void InitializeMasterDatabase(DatabaseProperties database)
        {
            ...
            {
                SettingsHelper.AppSettings[ValidationHelper.APP_SETTINGS_HASH_STRING_SALT] = DEFAULT_HASH_STRING_SALT;
                SqlInstallationHelper.InstallDatabase(connectionString, scriptPath, null, null, HandleDatabaseInstallationMessage, DB_SCHEMA);
            }
            finally
            {
                SqlInstallationHelper.AfterDataGet -= UpdateMacroSignatures;
        ...
        {
            ReleaseDatabases(mFixtureDatabase);
        ...
        {
            var csName = extraDbName + "CMSConnectionString";
